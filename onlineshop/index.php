...
if(isset($_POST['add_to_cart'])){
   ...
   $select_cart = mysqli_query($con, "SELECT * FROM `addcard` WHERE name = '$product_name' AND user_id = '$user_id'") or die('query failed');

   if(mysqli_num_rows($select_cart) > 0){
      $message[] = 'المنتج أضيف بالفعل إلى عربة التسوق!';
   }else{
      mysqli_query($con, "INSERT INTO `addcard`(user_id, name, price, image, quantity) VALUES('$user_id', '$product_name', '$product_price', '$product_image', '$product_quantity')") or die('query failed');
...
if(isset($_POST['update_cart'])){
   ...
   mysqli_query($con, "UPDATE `addcard` SET quantity = '$update_quantity' WHERE id = '$update_id'") or die('query failed');
...
if(isset($_GET['delete_all'])){
   mysqli_query($con, "DELETE FROM `addcard` WHERE user_id = '$user_id'") or die('query failed');
   header('location:index.php');
}
...
<div class="shopping-cart">
   ...
   <table>
      ...
      <tbody>
      <?php
         $cart_query = mysqli_query($con, "SELECT * FROM `addcard` WHERE user_id = '$user_id'") or die('query failed');
         $grand_total = 0;
